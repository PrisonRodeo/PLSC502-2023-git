...
lw<-2.5 # line width
pdf("PL-2023-Histogram2.pdf",6,5)
par(mar=c(4,4,2,2))
with(PL, hist(Points,col="grey",main="",xlab="Points Total"))
...
legend("topright",bty="n",legend=c("Arithmetic Mean (6.8)",
       "Geometric Mean (0)","Harmonic Mean (0)","Median (6)","Mode (6)"),
       lwd=lw,lty=c(2,3,4,5,6),
       col=c("red","black","blue","darkgreen","orange"))
dev.off()
...
# Variation                              ####
#
# Hypothetical loser-to-winner scenario

hyp<-PL$Points
hyp[20]<-15 # Luton Town kicks butt...
...
print(PL[2,c(2,10)],row.names=FALSE)
print(PL[c(2,14),c(2,10)],row.names=FALSE)
